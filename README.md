...
🛠️ 環境建置 (Installation)
在 Windows 環境下編譯 Apple 底層的 lzfse 壓縮演算法需要 C++ 編譯器支援，請確保你已安裝  (需勾選「使用 C++ 的桌面開發」)。
...
🚀 啟動指南 (Usage)
將你的 iPhone 透過 USB 連接至電腦，並信任此電腦。
...
程式會自動在背景啟動 tunneld 守護行程，並自動掃描連接的設備。

在左側選單鎖定你的 iPhone 後，即可開始操作！
